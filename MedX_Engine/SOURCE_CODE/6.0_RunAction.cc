...
void MyRunAction::SingleData(const std::string& mergedFileName)
{
    ...
    for (Long64_t i = 0; i < tree->GetEntries(); ++i)
    {
        ...
        // Verificación detallada de cada valor para identificar la causa de la omisión
        if (numberOfPhotons == 0)       G4cout << "Zero Number of Photons" << G4endl;
        if (initialEnergy == 0)         G4cout << "Zero Initial Energy" << G4endl;
        if (sampleMass == 0)            G4cout << "Zero Sample Mass" << G4endl;
        if (edepValue == 0)             G4cout << "Zero Edep Value" << G4endl;
        if (std::isnan(radiationDose))  G4cout << "Radiation Dose is NaN" << G4endl;
