...
void MyRunAction::MergeRootFiles()
{
    ...
    std::vector<std::string> rootFiles = {"ROOT/Rad_0_t0.root", "ROOT/Rad_0.root"};

    // Add all ROOT files generated by threads to the merger
    for (const auto& fileName : rootFiles) {merger.AddFile(fileName.c_str());}

    // Set the final merged output file name
    std::string mergedFileName = directory + "/merged_output.root";
